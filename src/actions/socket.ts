...
export function gotPushToken(newPushToken: any) {
  return async (dispatch: Dispatch, getState: any) => {
    ...
    console.log( "🐷newPushDeviceId:", newPushDeviceId );
    if (newPushDeviceId) {
...
export function establishDeviceOff(): Promise<void> {
  return async (dispatch: Dispatch, getState: any) => {
    ...
    let configs: any = {
      ...
      onRegister(token: { token: any }) {
        ...
        dispatch({
          ...
          permission: 'granted',
          description: 'Called from PushNotification > onRegister'
        });
        if (!st.isAndroid) {
