...
import { setNoBackgroundAction, confirmAlert } from './auth';
import CONSTANTS, {
  SET_ALL_CONTACTS,
  SET_VOKE_CONTACTS,
        ...
        .then(permission => {
          ...
          if (permission === Permissions.AUTHORIZED && theme.isAndroid) {
            if (!force) {
              ...
              if (
                lastUpdated &&
                now - lastUpdated < CONSTANTS.REFRESH_CONTACTS_TIME
              ) {
          ...
          const myNumber = getState().auth.user.mobile
            ? getState().auth.user.mobile
            : null;
          const myNumberCompare = (myNumber || '')
            .replace(/[^0-9]/g, '')
            .substring(-10);

          if (
            permission === Permissions.NOT_ASKED ||
            ...
            Permissions.requestContacts()
              .then(contacts => {
                ...
                all = lodashMap(all, c => {
                  ...
                  const firstNameLetter =
                    getFirstLetter(c.givenName) || getFirstLetter(name);
                  const lastNameLetter =
                    getFirstLetter(c.familyName) || firstNameLetter;
                  return {
                    name,
              ...
              .catch(err => {
                ...
                if (err === Permissions.DENIED) {
                  Alert.alert(
                    i18n.t('error.contactsTitle'),
                    i18n.t('error.contactsDescription'),
                  );
                }
  ...
  return (dispatch, getState) =>
    new Promise(resolve => {
      ...
      const results = contacts.filter(c => {
        if (isOneLetter) {
          return (
            c.lastNameLetter === searchTextUpper ||
            c.firstNameLetter === searchTextUpper
          );
        }
...
export function uploadContacts(contacts = []) {
  return dispatch =>
    new Promise((resolve, reject) => {
      ...
      const contactChunks = lodashChunk(
        formattedContacts,
        CONSTANTS.CONTACT_CHUNKS,
      );
